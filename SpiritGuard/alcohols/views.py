...
def render_calculator(request):
    ...
    data = {
        'bac':  "{:.3f}".format(bac),
        'date_of_first_drink': "{d}-{m}-{y}".format(d=now.date().day, m=now.date().month, y=now.date().year),
        'date_of_last_drink': "{d}-{m}-{y}".format(d=then.date().day, m=then.date().month, y=then.date().year),
        'time_of_first_drink': "{h}:{m}".format(h=now.time().hour, m=now.time().minute),
        'time_of_last_drink': "{h}:{m}".format(h=then.time().hour, m=then.time().minute),
